...
async def lock_perm(c: Alita, m: Message):
    ...
    if not lock_type:
        await m.reply_text("locked 🔐 "(m, "locks.locks_perm_sp"))
        return
    ...
    if lock_type == "all":
        try:
            ...
            await m.reply_text("Ehh no  permission :)" ,m ("?heh"))
        await m.reply_text(f"locked all  🔒 ")
...
async def unlock_perm(c: Alita, m: Message):
    ...
    if not unlock_type:
        await m.reply_text(f"not valid lock")
        return
    ...
    if unlock_type == "all":
        try:
            ...
            await m.reply_text(tlang(m, "general.no_perm_admin"))
        await m.reply_text(f"unlocked all 🔓")
        await prevent_approved(m)
        return
